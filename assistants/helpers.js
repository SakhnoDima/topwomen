...
export const dropdownSearch = async (page, toggleElement, searchedText) => {
  await toggleElement.click();
  await page.waitForSelector('div[data-test="menu-container"]', {
    timeout: 10000,
  });
  const dropdownMenu = await page.$('div[data-test="menu-container"]');
  if (dropdownMenu) {
    // console.log("Dropdown menu founded");
    await page.waitForSelector('span[data-ev-label="menu_item"]', {
      timeout: 10000,
    });
    const dropdownMenuItems = await dropdownMenu.$$(
      'span[data-ev-label="menu_item"]'
    );
    for (let dropdownMenuItem of dropdownMenuItems) {
      const dropdownMenuItemText = await dropdownMenuItem.evaluate((el) =>
        el.textContent.trim()
      );
    ...
    if (!mainElement) {
      ...
      if (childElementSelector) {
        ...
        if (childElement) {
          ...
          console.error(
            `Child element "${childElementSelector}" in "${mainElementSelector}" not found`
          );
          return null;
        }
      }
      const elementText = await mainElement.evaluate((el) =>
        el.textContent.trim()
      );
      if (!elementText) {
      ...
      if (!elementMatch) {
        console.error(
          `No match found for text "${elementText}" with regex "${regex}"`
        );
        return null;
      }
      const elementString = Array.isArray(elementMatch)
        ? elementMatch[1]
        : elementMatch;
...
export const smoothMouseMoveAndClick = async (page, element, steps) => {
...
export const scrollToElement = async (page, element) => {
  await page.evaluate((el) => {
    el.scrollIntoView({
      behavior: "smooth",
      block: "center",
