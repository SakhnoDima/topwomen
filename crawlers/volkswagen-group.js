...
export async function fetchingDataFromVolkswagenGroup() {
    try {
        console.log("Volkswagen-group crawler started");
        let offset = 0;
        const vacancies = [];
        ...
        dataSaver("Volkswagen-group", vacancies);
    } catch (error) {
        trackMixpanel("Volkswagen-group", 0, false, error.message);
        console.error("Volkswagen-group crawler error:", error);
    }
}
