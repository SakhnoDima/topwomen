    ...
    try {
        ...
        while (pageCounter < pageCounterMax) {
            ...
            for (let i= 0; i < allVacancies.length; i++) {
                ...
                const vacancyTitle = await element.$eval('a', (el) => el.textContent.trim());
                ...
                if (vacancyTitle && vacancyId && jobLocation) {
                    ...
                    const vacancyLink = `https://hourlyjobs-us.mondelezinternational.com/job-reference/${await extractId(vacancyId)}`;
